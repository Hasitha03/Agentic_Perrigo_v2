...
def find_cost_savings(parameters, complete_input, rate_card):
    ...
    filter_data =get_filtered_data(parameters,complete_input)

    print(" RUNNING STATIC COST SAVING AGENT NODE ....  FILTERING DATA ")

    aggregated_data, total_shipment_cost = cost_of_columns(filter_data, rate_card)
    print(" RUNNING STATIC COST SAVING AGENT NODE ....  AGGREGATING DATA ")
    ...
    for k, v in scenarios.items():
        ...
        if current != k:
            st.toast(f"Consolidating for {k} days")
            current = k
    ...
    best_scenario = sorted_results.iloc[0]
    ...
    charts = create_heatmap_and_bar_charts(consolidated_data, aggregated_data, parameters['start_date'],
                                       parameters['end_date'])
    ...
    with st.expander("Heatmap Analysis Charts(Before & After Consolidation)"):
        ...
        for year in [2023,2024,2025]:
            if year in years_in_range:
                ...
                st.components.v1.html(chart_original.render_embed(), height=216, width=780)
                st.components.v1.html(chart_consolidated.render_embed(), height=216, width=780)

    return all_results,best_scenario